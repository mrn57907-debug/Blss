...
window._dmExtrasOpenMenu = function (otherId) {
  ...
  // تمييز بصري خفيف جداً للعنصر المحدَّد حتى إغلاق القائمة
  document.querySelectorAll(".dmex-highlighted").forEach(el => el.classList.remove("dmex-highlighted"));
  document.querySelector(`[data-other-id="${otherId}"]`)?.classList.add("dmex-highlighted");
};
...
(function _exInjectStyles() {
  ...
  style.textContent = `
    ...
    /* تمييز بصري خفيف جداً وشفاف للعنصر المحدَّد أثناء فتح قائمة الخيارات
       (دردشة أو عضو) — يختفي تلقائياً عند إغلاق القائمة */
    .dmex-highlighted {
